...
Because `AdminRoute` is a normal class in your project, you can tighten access beyond `trusted_roles` — for example, mounting it on a different base
path that's only reachable from an internal network, or overriding `trustedRoles` to a narrower list:
