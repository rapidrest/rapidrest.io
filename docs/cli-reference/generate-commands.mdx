...
| `--api [version]` | Use `@ApiRoute` instead of `@Route` (see [Routing & Controllers](/docs/routing-controllers/defining-routes#api-versioned-routes-using-apiroute)). Pass a version to prefix the path with `/api/v<version>`; bare `--api` prefixes with `/api` only |
