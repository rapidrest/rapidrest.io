...
| `port` / `listen_host` / `ssl` / `max_body_size` | Server bind address and TLS — see [HTTP Engine](/docs/http-engine) |
| `session` | Server-side session config — see [Sessions](/docs/http-engine/sessions) |
| `cors` | CORS allow-list (`cors.origins`) — see [Cookies & CORS](/docs/http-engine/cookies-cors) |
| `headers` | Extra response headers applied to every request — see [Cookies & CORS](/docs/http-engine/cookies-cors) |
