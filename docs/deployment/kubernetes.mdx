...
Just like Docker Compose, every datastore ends up as a `datastores__<name>__<key>` environment variable on the pod, sourced from a generated `ConfigMap`/`Secret` pair rather than typed in by hand. These are the same nconf keys from [Configuration](/docs/core-concepts/configuration), just populated by Kubernetes instead of Compose.
