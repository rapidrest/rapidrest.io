...
`/push` (and `/acls`, if you kept a database selected). These are thin subclasses of the framework's [Default Routes](/docs/default-routes) that you're
...
- [Default Routes](/docs/default-routes): what the generated `AdminRoute`, `MetricsRoute`, `OpenAPIRoute`, `StatusRoute` and `PushRoute` files actually do
