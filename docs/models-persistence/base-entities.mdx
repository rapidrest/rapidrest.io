...
Use a `RecoverableBase*Entity` when you want deletes to be reversible (records are flagged `deleted: true` rather than removed), and `BaseEntity`'s built-in `version` field gives you optimistic-locking updates for free through [Auto CRUD Routes](/docs/auto-crud-routes).
