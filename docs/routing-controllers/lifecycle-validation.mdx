...
Every request to a decorated handler runs through a fixed pipeline before your handler code executes, and again afterward before the response is sent:

```text
auth strategies → required roles → ACL permission check → @Validate → @Before → handler → @After
```

Authentication, role checks, and ACL permissions are covered in [Auth & RBAC](/docs/auth-rbac) — this page covers the four decorators you use directly on a handler.

## `@Validate(func)`

Runs a validation function before the handler. Throw to reject the request:
